...
/**
 * Interface implemented by {@link PortfolioManager}.
 */
public interface PortfolioSystem {
  ...
  */
   ...
   */
  List<Asset> getAssetsWithHoldings(String assetSymbol);
   ...
   * @return boolean to indicate if symbol is valid.
   */
   ...
   * MSFT), and the parameter amount is less than total units of the stock, we should sell the units
   * that maximise our profit. Remember some of the stock could have been purchased on different
   * dates and therefore have been purchased at different price points.
   *
   ...
   *        exchange name, exchange symbol, previous closing value, opening value, gain/loss
   *        since opening. Add any additional data you feel is relevant.
   */
   ...
   */
  List<AssetQuote> getAssetQuotes(List<String> assetSymbols);
   ...
   */
  AssetQuote getAssetQuote(String assetSymbol);

  /** Get the available funds in the portfolio.

   * @return the available 'cash' balance available. This the balance of the funds used to
   *        purchase new assets. The proceeds of asset sales is added to this balance.
   */
